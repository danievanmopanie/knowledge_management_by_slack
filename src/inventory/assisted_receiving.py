...
class DeliveryDocumentParser:
    """Parse a structured CSV or table-like delivery note into delivery lines."""
...
class AssistedReceivingWorkflow:
    ...
    def __init__(
        ...
        parser: DeliveryDocumentParser | None = None,
    ) -> None:
    ...
    def _init_schema(self) -> None:
        with self.repository._connect() as conn:
            conn.execute(
                """CREATE TABLE IF NOT EXISTS inventory_staged_receipts (
    ...
    def stage(
        ...
        *,
        ...
        with self.repository.transaction() as conn:
            conn.execute(
                "INSERT INTO inventory_staged_receipts VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    receipt.receipt_id,
                    receipt.purchase_order_id,
                    receipt.delivery_id,
                    receipt.destination_location,
                    receipt.actor,
                    receipt.source_path,
                    json.dumps(payload),
                    receipt.created_at,
    ...
    def confirm(self, receipt_id: str, *, actor: str) -> str:
        with self.repository._connect() as conn:
            row = conn.execute(
                "SELECT * FROM inventory_staged_receipts WHERE receipt_id=?",
                (receipt_id,),
            ).fetchone()
        if row is None:
        ...
        with self.repository.transaction() as conn:
            conn.execute(
                "UPDATE inventory_staged_receipts SET status='confirmed' WHERE receipt_id=?",
                (receipt_id,),
            )
    ...
    def cancel(self, receipt_id: str, *, actor: str) -> str:
        with self.repository.transaction() as conn:
            row = conn.execute(
                "SELECT actor,status FROM inventory_staged_receipts WHERE receipt_id=?",
                (receipt_id,),
            ).fetchone()
            ...
            conn.execute(
                "UPDATE inventory_staged_receipts SET status='cancelled' WHERE receipt_id=?",
                (receipt_id,),
            )
    ...
    def _load_purchase_order(self, purchase_order_id: str) -> PurchaseOrder:
        with self.repository._connect() as conn:
            header = conn.execute(
                "SELECT * FROM inventory_purchase_orders WHERE purchase_order_id=?",
                (purchase_order_id,),
            ).fetchone()
            if header is None:
                raise InventoryDomainError(f"Unknown purchase order: {purchase_order_id}")
            rows = conn.execute(
                "SELECT * FROM inventory_po_lines WHERE purchase_order_id=? ORDER BY line_id",
                (purchase_order_id,),
            ).fetchall()
        return PurchaseOrder(
            ...
            status=PurchaseOrderStatus(header["status"]),
    ...
    def _delivery_payload(delivery: Delivery) -> dict:
        return {
            ...
            "lines": [asdict(line) for line in delivery.lines],
        }
        ...
        return Delivery(
            ...
            lines=[
                DeliveryLine(
                    ...
                    serial_numbers=tuple(item.get("serial_numbers", ())),
                    note=item.get("note", ""),
                )
    ...
    def _format_preview(po, delivery, result, receipt_id: str) -> str:
        ...
        lines = [
            f"*Receipt preview* `{receipt_id}` for PO `{po.purchase_order_id}`",
            f"Delivery lines: *{len(delivery.lines)}* | Accepted units: *{accepted}*",
        ]
        if result.discrepancies:
            lines.append("*Discrepancies:*" )
            for item in result.discrepancies:
                lines.append(f"• {item.discrepancy_type.value}: {item.detail}")
        else:
