...
class SerializedAssetLifecycleService:
    ...
    def put_away(self, asset_id: str, *, location_id: str, actor: str, note: str = "") -> SerializedAsset:
        ...
        return self._transition(asset_id, AssetLifecycle.IN_STOCK, actor=actor, location_id=location_id, clear_assignment=True, note=note)

    def issue(self, asset_id: str, *, assigned_to: str, customer_ref: str, allocation_type: AllocationType, actor: str, return_due_at: datetime | None = None, note: str = "") -> SerializedAsset:
        if not assigned_to:
        ...
        return self._transition(asset_id, AssetLifecycle.ISSUED, actor=actor, assigned_to=assigned_to, customer_ref=customer.customer_id, allocation_type=allocation_type, return_due_at=return_due_at, note=note)

    def return_asset(self, asset_id: str, *, actor: str, note: str = "") -> SerializedAsset:
        return self._transition(asset_id, AssetLifecycle.RETURNED, actor=actor, clear_assignment=True, note=note)

    def send_to_repair(self, asset_id: str, *, actor: str, location_id: str = "", note: str = "") -> SerializedAsset:
        return self._transition(asset_id, AssetLifecycle.REPAIR, actor=actor, location_id=location_id or None, clear_assignment=True, note=note)

    def quarantine(self, asset_id: str, *, actor: str, location_id: str = "", note: str = "") -> SerializedAsset:
        return self._transition(asset_id, AssetLifecycle.QUARANTINE, actor=actor, location_id=location_id or None, clear_assignment=True, note=note)

    def retire(self, asset_id: str, *, actor: str, note: str = "") -> SerializedAsset:
        return self._transition(asset_id, AssetLifecycle.RETIRED, actor=actor, clear_assignment=True, retired_at=datetime.now(timezone.utc), note=note)

    def dispose(self, asset_id: str, *, actor: str, note: str = "") -> SerializedAsset:
        if not self.profiles.can_dispose(asset_id):
            raise InventoryDomainError("A retired asset requires disposal evidence before it can be marked disposed.")
        if not self.losses.disposal_allowed(asset_id):
            raise InventoryDomainError(
                "A confirmed-lost asset requires an approved and completed financial write-off before disposal."
            )
        return self._transition(asset_id, AssetLifecycle.DISPOSED, actor=actor, clear_assignment=True, disposed_at=datetime.now(timezone.utc), note=note)

    def _transition(self, asset_id: str, target: AssetLifecycle, *, actor: str, location_id: str | None = None, assigned_to: str | None = None, customer_ref: str | None = None, allocation_type: AllocationType | None = None, return_due_at: datetime | None = None, clear_assignment: bool = False, retired_at: datetime | None = None, disposed_at: datetime | None = None, note: str = "") -> SerializedAsset:
        with self.repository.transaction() as conn:
            ...
            if target not in ALLOWED_TRANSITIONS.get(asset.status, set()):
                ...
                raise InventoryDomainError(f"Cannot move {asset.asset_id} from {asset.status.value} to {target.value}. Allowed: {allowed or 'none'}.")
            ...
            if clear_assignment:
                updates.update(assigned_to="", customer_ref="", allocation_type=None, return_due_at=None)
            if retired_at is not None:
                updates["retired_at"] = retired_at
            if disposed_at is not None:
                updates["disposed_at"] = disposed_at
            updated = replace(asset, **updates)
            movement = AssetMovement(movement_id=str(uuid4()), asset_id=asset.asset_id, from_status=asset.status, to_status=target, actor=actor, from_location=asset.location_id, to_location=updated.location_id, customer_ref=updated.customer_ref or asset.customer_ref, note=note)
